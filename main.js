(() => {
  ...
  // Interact stations
  const stations = [
    { id:'art', x:330, y:320, w:160, h:120, label:'Art Table 🎨 (E)', color:'#ffdfa6'},
    { id:'blocks', x:980, y:820, w:180, h:120, label:'Blocks Corner 🧱 (E)', color:'#ffd6e7'},
    { id:'music', x:1650, y:320, w:160, h:120, label:'Music Mat 🎵 (E)', color:'#d2ffd2'}
  ];
  ...
  function drawWorld(){
    ...
    stations.forEach(s => {
      if (completed[s.id]){
        ...
        ctx.fillText('✔', s.x + s.w - 18, s.y + 24);
      }
    });
  ...
  function loop(now){
    ...
    if (gameState === 'overworld'){
      ...
      drawWorld();
      promptText(['You had an awesome day at preschool! 🎉', 'Thanks for playing!']);
    }
  ...
  function drawMiniArt(){
    ...
    // mouse handling
    // simple once-per-click using pointer events
    ...
    if (artGame.done){
      ...
      ctx.fillText('Great coloring! ✔', 40, 160);
      awardIfFirst('art');
    }
    drawBackButton();
  }
  ...
  function drawMiniBlocks(dt){
    ...
    if (blocksGame.done){
      if (blocksGame.caught >= 10){
        ctx.fillStyle = '#2a9d8f'; ctx.font = '26px sans-serif';
        ctx.fillText('Awesome catching! ✔', 40, 410);
        awardIfFirst('blocks');
      } else {
        ctx.fillStyle = '#e76f51'; ctx.font = '20px sans-serif';
        ctx.fillText('Try again — press E to exit and re-enter.', 40, 410);
      }
    }
    drawBackButton();
  }
  ...
  function drawMiniMusic(){
    ...
    // status
    ctx.fillStyle = '#2a2a2a'; ctx.font = '18px sans-serif';
    if (musicGame.playing) ctx.fillText('Playing pattern…', 40, 90);
    else ctx.fillText(`Your turn: ${musicGame.input.length}/${musicGame.seq.length}`, 40, 90);

    if (musicGame.done){
      ctx.fillStyle = '#2a9d8f'; ctx.font = '26px sans-serif';
      ctx.fillText('Lovely music! ✔', 40, 460);
      awardIfFirst('music');
    }
